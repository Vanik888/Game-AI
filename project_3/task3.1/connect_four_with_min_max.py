...
class ConnectFourGame:
    def __init__(self,
        ...
        self.x_stat = x_stat
        self.o_stat = o_stat
        self.last_inserted_child_row = -1
        self.last_inserted_child_column = -1
    ...
    def make_move_for_children(self, q, p, column=None):
    ...
    """This method checks whether one player has won the game"""
    def move_was_3pre_winning_move_for_matrix(self, S, p):

        # Check all rows if there is a winning combination
        # for i in range(n_rows):
        for i in xrange(S.shape[0]):
            row_to_check = S[i, :]
            if self.__check_all_sublists_for_3(row_to_check, p):
                return True
        ...
        for i in xrange(S.shape[1]):
            column_to_check = S[:, i]
            if self.__check_all_sublists_for_3(column_to_check, p):
                return True
        ...
        for i in diagonal_shifts:
            diagonal = self.game_state.diagonal(i)
            if len(diagonal) >= winning_sum:
                if self.__check_all_sublists_for_3(diagonal, p):
                    return True
    ...
    """This method checks whether one player has won the game"""
    def move_was_2pre_winning_move_for_matrix(self, S, p):

        # Check all rows if there is a winning combination
        # for i in range(n_rows):
        for i in xrange(S.shape[0]):
            row_to_check = S[i, :]
            if self.__check_all_sublists_for_2(row_to_check, p):
                return True
        ...
        for i in xrange(S.shape[1]):
            column_to_check = S[:, i]
            if self.__check_all_sublists_for_2(column_to_check, p):
                return True
        ...
        for i in diagonal_shifts:
            diagonal = self.game_state.diagonal(i)
            if len(diagonal) >= winning_sum:
                if self.__check_all_sublists_for_2(diagonal, p):
                    return True
    ...
    """Constructs sublists of given list and
    check whether there is a winning combination for sublists with length 4"""
    def __check_all_sublists_for_3(self, list_to_check, p):
        sublists = [list_to_check[i:i + 4] for i in
                    range(0, len(list_to_check), 1)]

        for sublist in sublists:
            if len(sublist) < 4:
                continue
            if np.sum(sublist) * p == 3 and (np.array(sublist) == 0).sum() >= 1:
                return True
    ...
    """Constructs sublists of given list and
    check whether there is a winning combination for sublists with length 4"""
    def __check_all_sublists_for_2(self, list_to_check, p):
        sublists = [list_to_check[i:i + 4] for i in
                    range(0, len(list_to_check), 1)]
        ...
        for sublist in sublists:
            if len(sublist) < 4:
                continue
            if np.sum(sublist) * p == 2 and (np.array(sublist) == 0).sum() >= 2:
                ys = np.where(np.array(sublist) == 1 * p)[0]
                if ys[1] - ys[0] == 1:
                    return True
    ...
    def _utitlity_function(self, S, p):
        ...
        lastInserted_x, lastInserted_y = np.where(
            np.not_equal(S, self.game_state))
        self.last_inserted_child_row = lastInserted_x[0]
        self.last_inserted_child_column = lastInserted_y[0]

        if self.move_was_winning_move_for_matrix(S, p):
            return p
        if self.move_was_winning_move_for_matrix(S, p * (-1)):
            return p * (-1)
        if self.move_was_3pre_winning_move_for_matrix(S, p):
            return 0.9 * p
        if self.move_was_3pre_winning_move_for_matrix(S, p * (-1)):
            return 0.9 * p * (-1)
        if self.move_was_2pre_winning_move_for_matrix(S, p):
            return 0.7 * p
        if self.move_was_2pre_winning_move_for_matrix(S, p * (-1)):
            return 0.7 * p * (-1)
        if not self.move_still_possible_for_matrix(S):
            return 0

        xs_one, ys_one = np.where(S == 1)
    ...
    def check_block(self, S, p):
        ...
        if self.last_inserted_child_column >= 1 and row_to_check[self.last_inserted_child_column - 1] == p * -1:
            return True
        if self.last_inserted_child_column <= 5 and row_to_check[self.last_inserted_child_column + 1] == p * -1:
            return True
